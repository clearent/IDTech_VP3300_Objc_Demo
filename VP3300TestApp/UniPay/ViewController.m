...
//        }
       ...
       // [self appendMessageToResults:[NSString stringWithFormat:@"LCDDISPLAY %@", str ]];
    //}
//    NSLog(@"lcdDisplay");
//    NSLog(str);
//    switch (mode) {
...
static NSString *selectedDeviceId = nil;
...
- (void) bluetoothDevices:(NSArray<ClearentBluetoothDevice> *)bluetoothDevices {
    if(bluetoothDevices != nil) {
        for (ClearentBluetoothDevice* clearentBluetoothDevice in bluetoothDevices) {
            selectedDeviceId = clearentBluetoothDevice.deviceId;
...
static bool searchInProgress = NO;
    
- (void)startSearchWithUUID:(NSString *)uuid {
    if (searchInProgress && [uuid isEqualToString:deviceID]) {
        // Search already in progress for the specified UUID
        return;
    }
    // RETURN_CODE disableRet = [[IDT_VP3300 sharedController] device_disableBLEDeviceSearch];
    
   // [self appendMessageToResults:[NSString stringWithFormat:@"Enabling Search for BLE %@", uuid]];
    ...
    bool enableRet = [clearentVP3300 device_enableBLEDeviceSearch:val];
}
...
- (void)viewDidLoad
{
    ...
    //[friendlyName setText: [[IDT_VP3300 sharedController] device_getBLEFriendlyName]];
    [friendlyName setText: @"03826"];
    
    txtReceiptEmailAddress.text = @"dhigginbotham@clearent.com";
}
...
- (IBAction) f_searchForBLE:(id)sender{
    ...
    clearentConnection.bluetoothDeviceId = nil;
   // clearentConnection.bluetoothMaximumScanInSeconds = 45;
    clearentConnection.lastFiveDigitsOfDeviceSerialNumber = [friendlyName text];
    //clearentConnection.fullFriendlyName = @"IDTECH-VP3300-67467";
    //clearentConnection.fullFriendlyName = [ClearentConnection createFullIdTechFriendlyName:[friendlyName text]];
...
- (IBAction) f_noPin:(id)sender{
    runSampleAsRefund = YES;
    //Clearent..hijacked this to perform a refund transaction for certification
    ...
    // 9C20 resolves a refund issue
    [tagsDictionary setObject:@"20" forKey:@"9C"];
    
    NSData *tags = [IDTUtility encodeTags:tagsDictionary];
    RETURN_CODE rt = [clearentVP3300 device_startTransaction:amount amtOther:0 type:0 timeout:60 tags:tags forceOnline:false fallback:true];
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
        [self appendMessageToResults: @"Tap, insert, or swipe"];
    }
    else{
...
- (IBAction) configGroup0:(id)sender{
    ...
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
        [self appendMessageToResults: [NSString stringWithFormat:@"Confg Group 0 Tags:\n%@", result.description]];
...
- (void) recordICCReaderStatus {
    ICCReaderStatus* response;
       RETURN_CODE icc_getICCReaderStatusRt = [clearentVP3300 icc_getICCReaderStatus:&response];
       if(RETURN_CODE_DO_SUCCESS != icc_getICCReaderStatusRt){
           [self displayUpRet2: @"GetICCReaderStatus" returnValue: icc_getICCReaderStatusRt];
       }
    ...
    NSLog([NSString stringWithFormat:@"icc reader status %@",sta]);
...
-(IBAction)startEMV:(id)sender{
    ...
    if (RETURN_CODE_DO_SUCCESS == rt)
    {
       // [self appendMessageToResults: @"Insert or swipe"];
    }
    else{
        [self displayUpRet2: @"Start Transaction info" returnValue: rt];
    }
